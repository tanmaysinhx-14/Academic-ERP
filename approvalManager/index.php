...
<?php // ── Fetch all pending approval records (always — search is now client-side) ──
  try {
...
<?php // ── Delete approval record ─────────────────────────────────────────────────────
  ...
  // Re-fetch after any POST action so the table reflects the current state
  try {
    $STMT_retrieveApprovalsList = 'SELECT * FROM approval_users ORDER BY approval_rate_limiting_timestamp DESC';
    $retrieveApprovalsList      = $db1->prepare($STMT_retrieveApprovalsList);
    $retrieveApprovalsList->execute();
  ...
  $excludedColumns = ['approval_password', 'approval_IP_address'];
?>
...
<style>
  ...
  .ci-dialog__body   { padding: 1rem 1.25rem; }
  .ci-dialog__footer {
    display: flex;
    align-items: center;
  ...
  .ci-dialog__close {
    ...
    border: none; border-radius: .25rem; background: transparent
            ...
            <table class="table table-bordered table-striped table-sm align-middle text-center" id="approvalTable">
              ...
              <tbody id="approvalTableBody">
                ...
                <tr data-search-email="<?php echo strtolower($rowEmail); ?>"
                  ...
                  <?php foreach ($columns as $column): ?>
                    <td>
                      <?php echo htmlspecialchars((string) ($record[$column] ?? ''), ENT_QUOTES, 'UTF-8'); ?>
                    </td>
                  <?php endforeach; ?>
