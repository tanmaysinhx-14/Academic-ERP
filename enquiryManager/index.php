...
<?php // Backend for Enquiry Manager
  ...
  if (
    ...
    $selectedType     = sanitizeInput($_POST['enquiry_type'], 'alphanumeric');
    $selectedCategory = sanitizeInput($_POST['enquiry_category'], 'alphanumeric');
  }
  elseif (isset($_GET['type'], $_GET['category'])) {
    $selectedType     = sanitizeInput($_GET['type'], 'alphanumeric');
    $selectedCategory = sanitizeInput($_GET['category'], 'alphanumeric');
  }
  ...
  if (isset($_POST['submitEnquiryDetails'])) {
    $enquiryType     = sanitizeInput($_POST['enquiry_type'], 'alphanumeric') ?? null;
    $enquiryCategory = sanitizeInput($_POST['enquiry_category'], 'alphanumeric') ?? null;
    $csrfToken       = sanitizeInput($_POST['csrf_token'], 'alphanumeric') ?? null;
  ...
  if (isset($_POST['updateEnquiryStatus'])) {
    $record_id = sanitizeInput($_POST['record_id'] ?? '', 'number');
...
<?php if (checkForEquality(checkLoginStatus($db1), true, 'strict')): // User is logged in ?>
  <?php if(checkForEquality($enquiryFormStatus, 'selectManagerTask', 'strict')): ?>
    ...
    <section class="section-border border-primary ff-inter">
      <div class="container d-flex flex-column">
        <div class="row justify-content-center align-items-center gx-0 min-vh-100">
  ...
  <?php elseif(checkForEquality($enquiryFormStatus, 'submitEnquiryDetails', 'strict')): ?>
    ...
    <section class="section-border border-primary ff-inter">
      <div class="container d-flex flex-column">
        <div class="row justify-content-center gx-0 min-vh-100">
          <div class="col-12 col-lg-9 col-md-10 py-8 py-md-8">
