...
if (isset($_POST['createFacultyBtn'])) {
  ...
  $selectedBatches = isset($_POST['faculty_batches']) ? array_map('htmlspecialchars', $_POST['faculty_batches']) : [];
  $batchValue = json_encode($selectedBatches);
  if (validateCsrfToken($csrfToken)) {
    ...
    if (empty($selectedBatches)) {
      setToast('Please select at least one batch for this faculty member.', 'danger', 6000);
      $isValid = false;
    } elseif ($isValid && !validateEmail($enteredEmail)) {
      ...
      $isValid = false;
    } elseif ($isValid && checkUserRecord($db1, 'faculty_details', ['faculty_email' => $enteredEmail])) {
      $facultyEmailValidationStatus = 'is-invalid';
      $facultyEmailHelpText         = '<span class="text-danger d-flex align-items-center gap-1 mt-1">
                                          <span class="material-symbols-outlined" style="font-size:1rem;">info</span>
      ...
      while ($currentAttemptForInsertingFacultyData < $maxRetriesForInsertingFacultyData) {
        try {
          ...
          $STMT_insertFacultyConfig = 'INSERT INTO faculty_configurations (faculty_id, faculty_usercode, faculty_email, faculty_account_activation_status, faculty_batch_list, faculty_has_updated_username, faculty_has_updated_account_profile, faculty_has_opted_email_communication)
                                  VALUES (:faculty_id, :faculty_usercode, :faculty_email, 0, :faculty_batch_list, 0, 0, 1)';
          ...
          $insertFacultyConfig->bindValue(':faculty_batch_list',  $batchValue,      PDO::PARAM_STR);
...
if (isset($_POST['editFacultyBtn'])) {
  ...
  $editBatches  = isset($_POST['edit_faculty_batches']) ? array_map('htmlspecialchars', $_POST['edit_faculty_batches']) : [];
  $editBatchValue = json_encode($editBatches);
  $csrfToken      = escapeOutput($_POST['csrf_token'])                 ?? null;
    ...
    if ($isValid) {
      ...
      while ($currentAttempt < $maxRetries) {
        try {
          ...
          $updateFacultyConfig->bindValue(':batch_list',  $editBatchValue, PDO::PARAM_STR);
        ...
        <div class="row g-4 align-items-start">
                              ...
                              <?php if ($hasCredentials): ?>
                                <div class="d-flex align-items-center gap-2">
                                  <button type="button"
                                    ...
                                    data-batches="<?php echo $displayBatchValue; ?>"
          ...
          <div class="col-12 col-lg-5 col-xl-4">
            <form class="d-flex flex-column gap-4" method="POST" action="./">
              ...
              <div class="card border shadow-lg">
                <div class="card-header py-3">
                  <h4 class="mb-0 fw-semibold">Assigned Batches</h4>
                  <small class="text-body-secondary">Select the batches this faculty member teaches</small>
                </div>
                <div class="card-body p-4">
                  <?php if (!empty($activeBatchList)): ?>
                    <div class="border rounded-3 p-3 d-flex flex-column gap-2" style="max-height:220px;overflow-y:auto;">
                      <?php foreach ($activeBatchList as $batchCode): ?>
                        <div class="form-check">
                          <input class="form-check-input"
                            type="checkbox"
                            name="faculty_batches[]"
                            id="faculty_batch_<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>"
                            value="<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>"
                            <?php echo (isset($_POST['faculty_batches']) && in_array($batchCode, $_POST['faculty_batches'])) ? 'checked' : ''; ?> />
                          <label class="form-check-label fs-sm"
                            for="faculty_batch_<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <div class="d-flex gap-2 mt-2">
                      <button type="button"
                        class="btn btn-link btn-sm p-0 fs-xs text-decoration-none"
                        onclick="document.querySelectorAll('[name=\'faculty_batches[]\']').forEach(c => c.checked = true)">
                        Select All
                      </button>
                      <span class="text-body-tertiary fs-xs">·</span>
                      <button type="button"
                        class="btn btn-link btn-sm p-0 fs-xs text-decoration-none"
                        onclick="document.querySelectorAll('[name=\'faculty_batches[]\']').forEach(c => c.checked = false)">
                        Deselect All
                      </button>
                    </div>
                  <?php else: ?>
...
<dialog class="ci-dialog" id="editFacultyDialog" aria-labelledby="editDialogTitle">
  <form method="POST" action="./">
    ...
    <div class="ci-dialog__body">
      ...
      <div class="mt-3">
        <label class="form-label fw-semibold fs-sm mb-2">Assigned Batches</label>
        <?php if (!empty($activeBatchList)): ?>
          <div class="border rounded-3 p-3 d-flex flex-column gap-2"
            style="max-height:200px;overflow-y:auto;"
            id="edit_faculty_batch_list">
            <?php foreach ($activeBatchList as $batchCode): ?>
              <div class="form-check">
                <input class="form-check-input edit-faculty-batch-check"
                  type="checkbox"
                  name="edit_faculty_batches[]"
                  id="edit_faculty_batch_<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>"
                  value="<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>" />
                <label class="form-check-label fs-sm"
                  for="edit_faculty_batch_<?php echo htmlspecialchars($batchCode, ENT_QUOTES, 'UTF-8'); ?>">
                  <?php echo htmlspecialchars(formatBatchLabel($batchCode), ENT_QUOTES, 'UTF-8'); ?>
                </label>
              </div>
            <?php endforeach; ?>
          ...
          <div class="d-flex gap-2 mt-2">
            <button type="button"
              class="btn btn-link btn-sm p-0 fs-xs text-decoration-none"
              onclick="document.querySelectorAll('.edit-faculty-batch-check').forEach(c => c.checked = true)">
              Select All
            </button>
            <span class="text-body-tertiary fs-xs">·</span>
            <button type="button"
              class="btn btn-link btn-sm p-0 fs-xs text-decoration-none"
              onclick="document.querySelectorAll('.edit-faculty-batch-check').forEach(c => c.checked = false)">
              Deselect All
            </button>
          </div>
        <?php else: ?>
          <div class="text-body-secondary fs-sm border rounded-3 p-3">
            <span class="material-symbols-outlined align-middle" style="font-size:1rem;">info</span>
            No active batches available. Activate batches from the Batchlist Manager first.
          </div>
        <?php endif; ?>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  function openEditModal(btn) {
    ...
    document.querySelectorAll('.edit-faculty-batch-check').forEach(chk => {
